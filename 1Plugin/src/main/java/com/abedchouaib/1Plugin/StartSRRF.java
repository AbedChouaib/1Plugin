...
public class StartSRRF implements PlugIn
{
...
//	String OutputCAC = "-CAC_OUTPUT";
	...
	private void StartAnalysis(File[] FileList, int i, String Output, boolean GenerateLog, boolean SwitchChannel, int ChnPos)
	{
		...
		try
		{
			...
			if (SpecChn != 0)
			{
				DriftTablePath = GetFixedDriftTablePath(OImg, ImageName, ChannelNames, OutResults, OutSRRF, SpecChn);
			}
					...
					if (chn <= channels)
					{
						...
						if (SpecChn == 0)
						{
							if (SwitchChannel)
							{
								...
								LogFile.add("Drift reference channel was channed from " + (ChnPos + 1) + " --> to " + (currentChn + 1));
							}
							DriftTablePath = GetDriftTablePath(OImg[currentChn], PathAndName);
						}
						ImgSRRF[k] = SRRFanalysis(OImg[k], PathAndName, 0, SRRF_MaxImgNum, DriftTablePath);
						if (jump)
						{
						...
						ChannelNames[k] = "SRRF_" + ChannelNames[k];
...
//						HyperImg.resetDisplayRange();
				...
				} else
				{
					...
					{
						IJ.run("Close All");
						StartAnalysis(FileList, i, OutputDir, false, true, ChnPos);
					} else
					{
...
//			IJ.run("Close All");
	...
	private String GetDriftTablePath(ImagePlus Img, String[] PathAndName)
	{
		...
		Options = Options + " show_cross-correlation choose=[" + drift_table_path + "]";
//		IJ.selectWindow(Img.getTitle());
		// IJ.showMessage("Image name " + Img.getTitle()+ "Image path " +
		// drift_table_path);
	...
	private ImagePlus SRRFanalysis(ImagePlus Img, String[] PathAndName, int Start, int End, String table_path)
	{
		...
		OptionsSRRF = OptionsSRRF + Radiality_RPC + Radiality_Renorm + Radiality_DoGS + Weighting_DoIW + Weighting_DoGW + psf + minSRRFPat;
...
//		IJ.run("Next Slice [>]");
	 ...
	 */
	String GetFixedDriftTablePath(ImagePlus[] OImg, String ImageName, String[] ChannelNames, String OutResults, String OutSRRF, int chn)
	{
		int k = SpecChn - 1;
		ChannelNames[k] = ImageName + "_" + chn;
		OImg[k].setTitle(ChannelNames[k]);
//		OImg[k].createImagePlus(); // <<<<<<<<<<<<<<<<<<<<<<<< show/hide.
		String[] PathAndName = { ChannelNames[k], OutResults, OutSRRF };
