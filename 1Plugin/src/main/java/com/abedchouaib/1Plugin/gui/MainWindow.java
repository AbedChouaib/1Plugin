...
import com.abedchouaib.ONEMicroscopy.StartSRRF;
...
//	static String FileExtension;
	...
	public static ArrayList<int[]> CropMaskListValues = new ArrayList<int[]>();
...
//		Frame.setIconImage(mainWindoIcon.getImage());
		...
		lblRadMag = new JLabel("Radiality magnification (1-10)");
...
//				BeadsChannels
		...
		panel_1.setBounds(456, 26, 161, 199);
		...
		btn_AdOpt.setBounds(10, 165, 141, 23);
...
//								frmOneMicroscopy.setVisible(false);
								if (SaveButtonGaurd)
								{
									if (!SaveParentFolder)
									{
										if (username != null)
										{
											File OutDir = new File(username + File.separator + "Desktop" + File.separator + "ONE Analysis"
													+ File.separator);
...
//								BeadsDirPath = temp1.isBlank() ? "" : temp1;
								BeadsDirPath = temp1;
								String temp2 = ImportF.getText();
//								ImportDirPath = temp2.isBlank() ? "" : temp2;
...
//============================================== Mask Creation Window ==============================================
	...
	void LoadPrefs()
	{
		...
		SRRFType = Prefs.get(AdOptPref + "SRRFType", SRRFTypes[1]);
		...
		Radiality_RPC = Prefs.get(AdOptPref + "removeRadialityPositivityConstraint", false);
		Radiality_Renorm = Prefs.get(AdOptPref + "renormalize", false);
		Radiality_DoGS = Prefs.get(AdOptPref + "doGradSmooth", false);

		Weighting_DoIW = Prefs.get(AdOptPref + "doIntensityWeighting", true);
		Weighting_DoGW = Prefs.get(AdOptPref + "doGradWeight", false);
	...
	{
		GenericDialog GD = new GenericDialog("SRRF - Advance Settings");
		GD.addCheckbox("Integrate_Temporal_Correlations (default: active)", integrate_temporal_correlations);

		GD.addMessage(" 		-- Radiality --		");
		GD.addCheckbox("Remove_Positivity_Constraint (default: disabled)", Radiality_RPC);
		GD.addCheckbox("Renormalize (default: disabled, activate for 2D structures)", Radiality_Renorm);
		GD.addCheckbox("Do_Gradient_Smoothing (default: disabled, activate in low-density)", Radiality_DoGS);

		GD.addMessage(" 		-- Weighting --		");
		GD.addCheckbox("Do_Intensity_Weighting (default: active)", Weighting_DoIW);
		GD.addCheckbox("Do_Gradient_Weighting (default: disabled, activate in low-SNR, unstable)", Weighting_DoGW);
		GD.addSlider("PSF_FWHM (needed in Gradient Weighting)", 1.00f, 5.00f, psf_fwhm);
